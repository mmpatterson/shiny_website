...
server <- function(input, output, session) {
    ...
    switch(current_section(),
      "About Me" = tagList(
        div(
          ...
          div(
            style = "width: 60%; display: flex; gap: 10px; flex-wrap: wrap;",
            div(style = "flex: 1 1 40%; min-width: 250px;", leafletOutput("cityMap")),
            div(style = "flex: 1 1 40%; min-width: 250px;", imageOutput("skiing")),
            div(style = "flex: 1 1 40%; min-width: 250px;", imageOutput("sabres")),
            div(style = "flex: 1 1 40%; min-width: 250px;", imageOutput("goose")),
            div(style = "flex: 1 1 40%; min-width: 150px;", imageOutput("bills"))
          )
        )
      ),
      "Projects & Publications" = tagList(
        ...
        div(class = "project-grid",
          card(
            ...
            card_footer(tags$a(href = "https://journals.asm.org/doi/10.1128/msystems.01471-24", target = "_blank", "View Paper"))
          ),
          card(
            full_screen = FALSE,
            ...
            card_footer(tags$a(href = "data/object_detection_project.pdf", target = "_blank", "View Paper"))
          ),
          card(
            full_screen = FALSE,
            ...
            card_footer(tags$a(href = "https://youtu.be/yavHEWpgrCQ?si=MQ_OMf8dLCf1rdM9", target = "_blank", "View Presentation"))
          ),
          card(
            full_screen = FALSE,
            ...
            card_footer(tags$a(href = "https://mmpatterson.github.io/leaflet-earthquake-tracker/earthquake-tracker/", target = "_blank", "View Project"))
          )
        )
      ),
      "Resume" = tagList(
  ...
  output$bills <- renderImage({
    list(src = './www/Bills_Photo.jpeg', contentType = 'image/jpeg',
         width = "40%", height = "50%", alt = "A picture of my wife and me at a Bills game")
  }, deleteFile = FALSE)

  output$goose <- renderImage({
    list(src = './www/goose.jpeg', contentType = 'image/jpeg',
         width = "60%", height = "70%", alt = "My dog, Goose")
  }, deleteFile = FALSE)

  output$sabres <- renderImage({
    list(src = './www/sabres_game.jpeg', contentType = 'image/jpeg',
         width = "60%", height = "70%", alt = "A picture of my wife and me at a Sabres game")
  }, deleteFile = FALSE)

  output$skiing <- renderImage({
    list(src = './www/skiing.jpeg', contentType = 'image/jpeg',
         width = "60%", height = "70%", alt = "A picture of my wife, Katy, and me skiing in Vermont")
  }, deleteFile = FALSE)
}
